...
The system SHALL treat each product variant as a distinct sellable option with its own attributes, price, and stock.

#### Scenario: Admin manages variants
- GIVEN an Admin edits an existing or new variant
- WHEN valid variant data is submitted
- THEN the system stores the variant with its own price and stock under the product
